 ...
 """,
    ...
    async def on_upload_complete(
        ...
        if self.is_yt:
            ...
            if getattr(Config, "LEECH_DUMP_CHAT", getattr(Config, "LEECH_LOG_CHAT", None)):
                dump_chat = getattr(Config, "LEECH_DUMP_CHAT", Config.LEECH_LOG_CHAT)
                await send_message(int(dump_chat) if str(dump_chat).lstrip('-').isdigit() else dump_chat, msg, button)
            ...
            if not files and not self.is_super_chat:
                ...
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    dump_chat_val = getattr(Config, "LEECH_DUMP_CHAT", getattr(Config, "LEECH_LOG_CHAT", None))
                    if Config.MEDIA_STORE and (
                        self.is_super_chat or dump_chat_val
                    ):
                        parts = link.split("/")[-2:]
                        if len(parts) == 2:
        ...
        if self.pm_msg and (not Config.DELETE_LINKS or getattr(Config, "CLEAN_LOG_MSG", False)):
            await delete_message(self.pm_msg)

        # Removed 'await delete_links(self.message)' as requested.
